import numpy as np
import sklearn
from feature_extract import multiple_img_features
from PIL import Image

data=input('请在此处粘贴txt文件地址（去掉引号）')
...
filename=input('请在此处粘贴图片文件地址(去掉引号)')
img=Image.open(filename)
data=multiple_img_features(img)
x_test1=np.array([list(map(int,data))])
